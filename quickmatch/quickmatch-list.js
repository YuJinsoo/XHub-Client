window.onload = function() {
    // 미팅 전체 목록 가져오기
    // exercisehub.xyz
    axios.get('https://exercisehub.xyz/quickmatch/list/')
    ...
    document.getElementById('searchBtn').addEventListener('click', function() {
        ...
        axios.get('https://exercisehub.xyz/quickmatch/search/', { params: { search: searchTerm } })
            .then(response => populateMeetingList(response.data))
            .catch(error => console.error('Error fetching meetings:', error));
    });
}
...
function populateMeetingList(data) {
    ...
    data.forEach(meeting => {
        ...
        //     axios.post('https://exercisehub.xyz/quickmatch/join/', { meeting_id: meeting.id })
        //     .then(response => {
